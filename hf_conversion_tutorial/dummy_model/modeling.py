...
class RotaryPositionalEmbeddings(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        thetas = torch.tensor(
            [
                1 / (config.rope_theta ** ((2 * i) / head_size))
                for i in range(head_size // 2)
            ]
        )
        pos = torch.arange(max_seq_len)
        m_theta = torch.einsum("n, d -> nd", pos, thetas)
        m_theta = torch.stack([m_theta, m_theta], dim=-1).reshape(
...
class SelfAttention(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        if config.hidden_size % config.num_attention_heads != 0:
            raise ValueError(
                f"The QK size {config.qk_size} is not a multiple of the number of attention heads {config.num_attention_heads}"
            )
        ...
        self.in_proj_qkv = nn.Linear(
            self.hidden_size, 3 * self.hidden_size, bias=config.attention_bias
        )
    ...
    def initialize(self, attention_bias: bool):
        ...
        nn.init.trunc_normal_(
            self.in_proj_qkv.weight, mean=0.0, std=std, a=-2 * std, b=2 * std
        )
        nn.init.trunc_normal_(
    ...
    def attention_operation(
        ...
        """ """
        attention_probs: torch.Tensor = torch.matmul(
            query, key.transpose(2, 3) * self.scale
        )
    ...
    def forward(
        ...
        if attention_mask is None:
            attention_mask = (
                torch.ones(
                    query_len, key_len, dtype=torch.bool, device=hidden_states.device
                )
                .triu(diagonal=1)
                .unsqueeze(0)
                .unsqueeze(0)
        ...
        query, key, value = self.in_proj_qkv(hidden_states).chunk(
            3, dim=-1
        )  # shape: [B, S, D]
        ...
        key = key.reshape(
            batch_size, key_len, self.num_heads, self.head_size
        ).transpose(1, 2)
        value = value.reshape(
            batch_size, key_len, self.num_heads, self.head_size
        ).transpose(1, 2)
        ...
        attention_output, attention_probs = self.attention_operation(
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.mlp = nn.Sequential(
            nn.LayerNorm(
                hidden_size,
                eps=config.layernorm_eps,
                elementwise_affine=config.mlp_layernorm_learned,
            ),
    ...
    def __init__(self, config: ModelConfig, word_embedding: nn.Parameter):
        ...
        self.lm_head = nn.Sequential(
            nn.LayerNorm(
                hidden_size,
                eps=config.layernorm_eps,
                elementwise_affine=config.lm_head_layernorm_learned,
            ),
            nn.Linear(
                hidden_size, word_embedding.size(0)
            ),  # word_embedding.size(0) == vocab_size
        )
...
class Embedding(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.word_norm = nn.LayerNorm(
            config.hidden_size,
            eps=config.layernorm_eps,
            elementwise_affine=config.embedding_layernorm_learned,
        )
    ...
    def forward(self, input_tokens: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        word_embeddings = self.dropout(
            self.word_norm(self.word_embedding(input_tokens))
        )
...
class Layer(nn.Module):
    ...
    def forward(
        self: Layer, x: torch.Tensor, attention_mask: torch.Tensor
    ) -> torch.Tensor:
        """ """
...
class MyModelPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            nn.init.trunc_normal_(
                module.weight.data, mean=0.0, std=std, a=-2 * std, b=2 * std
            )
...
class MyModel(MyModelPreTrainedModel):
    ...
    def get_contextualized_embeddings(
        ...
        if attention_mask is None:
            attention_mask = (
                input_ids.new_ones((seq_length, seq_length), dtype=torch.bool)
                .triu(diagonal=1)
                .unsqueeze(0)
                .unsqueeze(0)
            )
        ...
        if attention_mask is not None:
            ...
            attention_mask = attention_mask | input_ids.new_ones(
                (seq_length, seq_length), dtype=torch.bool
        ...
        for layer in self.layers:
            layer_embeddings, layer_attention_probs = layer(
                contextualized_embeddings[-1], attention_mask
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        sequence_output, contextualized_embeddings, attention_probs = (
...
class MyModelForCausalLM(MyModelPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        sequence_output, contextualized_embeddings, attention_probs = (
            self.model.get_contextualized_embeddings(input_ids, attention_mask)
        )
