...
def _parse_arguments():
    ...
    # Required parameters
    parser.add_argument(
        "--data_path", default="", type=pathlib.Path, help="Path to the data directory"
    )
    parser.add_argument(
        "--output_dir",
        default="results",
    ...
    parser.add_argument(
        ...
        default="blimp",
        type=str,
        help="The task that is being evaluated.",
    ...
    parser.add_argument(
        ...
        default=0.01,
        type=float,
        help="Minimum temperature to apply to the logits.",
    )
    parser.add_argument(
        "--max_temperature",
        default=1.01,
        type=float,
        help="Maximum temperature to apply to the logits. If None, onlny the minimum temperature will be considered.",
    )
    parser.add_argument(
        "--temperature_interval",
        default=0.1,
        type=float,
        help="Step size between temperatures applied to the logits.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=64,
        type=int,
        help="Mini-batch size to process each batch of inputs involving masked tokens",
    )
    parser.add_argument(
        "--save_predictions",
        action="store_true",
...
def process_results(args: argparse.ArgumentParser, results: dict):
    ...
    for temp, temp_results in results.items():
        for subdomain, count_dict in temp_results.items():
            ...
            for key in keys:
                ...
                incorrect += count_dict["total"][key] - count_dict["correct"][key]

            subdomain_accs = {
                key: 100.0 * count_dict["correct"][key] / count_dict["total"][key]
                for key in keys
            }
    ...
    if args.task in ["blimp", "ewok", "wug"]:
        for temp, accuracy in accuracies.items():
            average_accuracies[temp] = sum(accuracy["UID"].values()) / len(
                accuracy["UID"].values()
            )
    else:
        ...
        for temp, subdomain_dict in accuracies.items():
            ...
            for split in splits:
                ...
                curr_acc = sum([split_dict[key] for key in split_keys]) / len(
                    split_keys
                )
...
def main():
    ...
    create_evaluation_report(
        best_temp, average_accuracies[best_temp], accuracies[best_temp]
    )
    with (args.output_path / "best_temperature_report.txt").open("w") as f:
        create_evaluation_report(
            best_temp, average_accuracies[best_temp], accuracies[best_temp], file=f
        )
